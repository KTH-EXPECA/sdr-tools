...
3. Verify that the link is up and `mtu` is 9000 on both ends
    ...
    Enable jumbo frames on the switch, make sure you reboot the switch after enabling the option.
...
2. Check that the OS network buffers are adjusted
 ...
 4. Disable power management states of CPU
    ```
    sudo apt install linux-tools-common linux-tools-5.4.0-155-generic
